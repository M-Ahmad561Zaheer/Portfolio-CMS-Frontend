...
function Header({ profile }) {
  ...
  const links = navItems.filter((item) => item === "home" || item === "contact" || isOn(profile, `${item}Enabled`));
...
export default function Home() {
  ...
  return <main className="portfolio-page"><Header profile={profile}/>
    ...
    {(isOn(profile,"blogEnabled") && blogs.length > 0) || (profile.exploringEnabled && exploring.length > 0) ? <section id="blog" className="section-block tint"><div className={container}>{isOn(profile,"blogEnabled") && blogs.length > 0 && <><div className="section-heading"><p className="kicker">Writing</p><h2>Latest Insights</h2><p>Notes from projects, problems and things I am learning.</p></div><div className={`blog-grid count-${Math.min(blogs.length,3)}`}>{blogs.slice(0,3).map((post) => <article className="blog-card" key={post.id}>{post.thumbnail ? <img src={post.thumbnail} alt="" loading="lazy"/> : <div className="blog-placeholder"><BookOpen/></div>}<div><time>{new Date(post.createdAt).toLocaleDateString()}</time><h3>{post.title}</h3><p>{post.content}</p><span className="text-link">Read More <ArrowRight/></span></div></article>)}</div></>}{profile.exploringEnabled && exploring.length > 0 && <div className="exploring"><div><p className="kicker">Growth</p><h2>Currently Exploring</h2><p>Ideas and technologies I am actively learning—not claiming to have mastered.</p></div><div>{exploring.map((item) => <span key={item}><Sparkles/>{item}<small>{profile.exploringStatus || "Learning"}</small></span>)}</div></div>}</div></section> : null}
